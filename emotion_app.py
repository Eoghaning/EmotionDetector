...
class EmotionApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        for i, (text, mode) in enumerate(buttons):
            ...
            btn = ctk.CTkButton(
                ...
                fg_color=colors[i] if mode != 8 else "#1F6AA5",
                hover_color="#3B3B3B" if mode != 8 else "#2D89D0"
            )
            btn.grid(row=row, column=col, padx=5, pady=5)
        
        self.mode_label = ctk.CTkLabel(
            self,
            text=f"Current Mode: {mode_names[self.current_mode]}",
    ...
    def set_mode(self, mode):
        ...
        self.mode_label.configure(text=f"Current Mode: {mode_names[mode]}")
        
        buttons = self.winfo_children()[1].winfo_children()
        for i, btn in enumerate(buttons):
            modes = [1, 2, 3, 4, 5, 6, 7, 8]
            if modes[i] == mode:
                btn.configure(fg_color="#1F6AA5", hover_color="#2D89D0")
            else:
                btn.configure(fg_color="#2B2B2B", hover_color="#3B3B3B")
